    ...
    <main>
      ...
      {view === 'month' ? <MonthView cursor={cursor} items={visible} onNew={openNew} onEdit={item => { setError(''); setModal({ type: 'intervention', item, date: item.startAt.slice(0, 10) }); }} onMove={moveIntervention} draggingId={draggingId} setDraggingId={setDraggingId} /> : <section className={`calendar ${view === 'day' ? 'day-view' : ''}`}>
        ...
        <div className="calendar-body">
          ...
          {(view === 'day' ? [cursor] : days).map(day => <DayColumn key={dateKey(day)} day={day} items={visible.filter(item => item.startAt.slice(0, 10) === dateKey(day))} allItems={visible} onNew={start => openNew(day, start)} onContextRequest={(start, x, y) => setContextMenu({ day, start, x: Math.min(x, window.innerWidth - 230), y: Math.min(y, window.innerHeight - 70) })} onEdit={item => { if (Date.now() < suppressEditUntil.current) return; setError(''); setModal({ type: 'intervention', item, date: dateKey(day) }); }} onMove={moveIntervention} onResize={startResize} draggingId={draggingId} setDraggingId={setDraggingId} dropPreview={dropPreview} setDropPreview={setDropPreview} />)}
        </div>
      </section>}
    </main>
    {contextMenu && <div className="context-menu" style={{ left: contextMenu.x, top: contextMenu.y }} onClick={event => event.stopPropagation()}><button onClick={() => { openNew(contextMenu.day, contextMenu.start); setContextMenu(null); }}><span>＋</span> Créer une intervention le {contextMenu.day.toLocaleDateString('fr-FR')} à {contextMenu.start}</button></div>}
...
function DayColumn({ day, items, allItems, onNew, onContextRequest, onEdit, onMove, onResize, draggingId, setDraggingId, dropPreview, setDropPreview }) {
